...
Type* new_array(Type* right_type, int size, int element_size); //新建ARRAY节点，right_type为右侧的类分析（分析是由右至左的）
Type* new_struct(); //穿件新的STRUCTURE空节点
void add_struct_member(Type* struct_now, char* name, Type* ty); //在struct_now中追加新的域（末端）
Func* new_fault_func(); //错误返回
Func* new_func(char *name, Type *return_type); //创建新的函数节点
Arg* new_arg(Type *arg_type); //创建新的参数节点
...
int getTypeSize(Type* ty); //获取类型的大小
